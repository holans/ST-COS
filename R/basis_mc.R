draw_spt_basis_mc <- function(R, domain, period_len, report.period = 100)
{
	n <- nrow(domain)
	T <- period_len
...
draw_sp_basis_mc <- function(R, domain, report.period = 100)
{
	n <- nrow(domain)
	s1 <- matrix(NA, n, R)
	s2 <- matrix(NA, n, R)
...
compute_spt_basis_mc <- function(basis, domain, period, s1, s2, report.period = 100)
{
	n <- nrow(domain)
	r <- basis$get_dim()
	R <- dim(s1)[3]
...
compute_sp_basis_mc <- function(basis, domain, s1, s2, report.period = 100)
{
	n <- nrow(domain)
	r <- basis$get_dim()
	R <- dim(s1)[2]
