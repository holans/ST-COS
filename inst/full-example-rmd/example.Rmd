...
```{r}
K <- solve(K.inv)
# old.out <- mle.stcos.old(z.scaled, v.scaled, H, S.reduced, init = list(sig2xi = 1))
...
```{r}
gibbs.out <- gibbs.stcos.raw(z.scaled, v.scaled, H, S.reduced, K.inv, R = 5000,
