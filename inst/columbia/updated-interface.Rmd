...
- The areal basis function classes make use of the point-level basis function classes `SpaceTimeBisquareBasis` and `SpatialBisquareBasis`, which remain exposed to users as well.

- Coordinating dimension reduction for `S` on the source and fine-level supports seems awkward in the old code. The new code asks the user to handle this part entirely, but makes it much simpler in my opinion.
